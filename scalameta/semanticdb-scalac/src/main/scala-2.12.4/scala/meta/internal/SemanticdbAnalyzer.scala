...
trait SemanticdbAnalyzer extends NscAnalyzer with ReflectionToolkit {
  ...
  import statistics.{appStart, failedApplyNanos, failedOpEqNanos, opeqStart, start, typedApplyCount, typedSelectCount}
  ...
  class SemanticdbNamer(context0: Context) extends Namer(context0) {
    override def selfTypeCompleter(tree: Tree) = new SemanticdbSelfTypeCompleter(tree)
    class SemanticdbSelfTypeCompleter(tree: Tree) extends SelfTypeCompleter(tree) {
  ...
  class SemanticdbTyper(context0: Context) extends Typer(context0) {
    ...
    import StatisticsStatics.areSomeColdStatsEnabled
    ...
    override def typed1(tree: Tree, mode: Mode, pt: Type): Tree = {
      ...
      def tryTypedApply(fun: Tree, args: List[Tree]): Tree = {
        val start = if (areSomeColdStatsEnabled) statistics.startTimer(failedApplyNanos) else null

        def onError(typeErrors: Seq[AbsTypeError], warnings: Seq[(Position, String)]): Tree = {
          if (areSomeColdStatsEnabled) statistics.stopTimer(failedApplyNanos, start)
      ...
      def normalTypedApply(tree: Tree, fun: Tree, args: List[Tree]) = {
        ...
        val appStart = if (areSomeColdStatsEnabled) statistics.startTimer(failedApplyNanos) else null
        val opeqStart = if (areSomeColdStatsEnabled) statistics.startTimer(failedOpEqNanos) else null
        ...
        def onError(error: SilentTypeError): Tree = fun match {
          case Select(qual, name) if isConversionCandidate(qual, name) =>
            ...
            if (treeInfo.isVariableOrGetter(qual1)) {
              if (areSomeColdStatsEnabled) statistics.stopTimer(failedOpEqNanos, opeqStart)
              ...
              if (areSomeColdStatsEnabled) statistics.stopTimer(failedApplyNanos, appStart)
        ...
        silentResult match {
          case SilentResultValue(fun1) =>
            ...
            if (areSomeColdStatsEnabled) statistics.incCounter(typedApplyCount)
            val noSecondTry = (
                 isPastTyper
              || context.inSecondTry
      ...
      def typedSelectOrSuperCall(tree: Select) = tree match {
        ...
        case Select(qual, name) =>
          if (areSomeColdStatsEnabled) statistics.incCounter(typedSelectCount)
            ...
            if (original != EmptyTree && pt != WildcardType) (
              ...
              orElse { _ =>
                ...
                typerWithLocalContext(resetContext) { typer1 =>
                  ...
                  if (tree1.isEmpty) tree1 else typer1.asInstanceOf[SemanticdbTyper].adapt(tree1, mode, pt, EmptyTree)
                }
              }
            )
            else
